...
printf("#############################################\n");
	...
	printf("Usage: linux-serial-test [OPTION]\n"
		...
		//	"  -s, --stats       Dump serial port stats every 5s\n"
			"  -s, --stats       Dump serial port stats every second (default is 0 meaning no limit)\n"
...
printf("#############################################\n");
	...
	for (;;) {
		...
		case 's':
			_cl_stats = 1;
			//char *endptr;
			//_cl_s_time = strtol(optarg, &endptr, 0);
			_cl_s_time =2;
			break;
		case 'S':
			_cl_stop_on_error = 1;
...
printf("#############################################\n");
	...
	_write_size = (_cl_tx_bytes == 0) ? 95 : _cl_tx_bytes;
	...
	while (!(_cl_no_rx && _cl_no_tx)) {
		...
		if (retval == -1) {
			...
			printf("serial_poll.revents:%d, POLLIN:%d\n", serial_poll.revents, POLLIN);
			...
			printf("serial_poll.revents:%d, POLLOUT:%d\n", serial_poll.revents, POLLOUT);
		...
		if (_cl_stats) {
			if (current.tv_sec - last_stat.tv_sec > _cl_s_time) {
				dump_serial_port_stats();
				last_stat = current;
			}
		}
...
int  main11()
{
	printf("Android serial test app\n");
	int argc;
	char *argv[] = {"serialtest", "-s", "-e", "-p", "/dev/ttyS2", "-c", "-l", "250"};
